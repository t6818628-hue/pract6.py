            ...
            with open("books.txt", "r", encoding="utf-8") as file:
                ...
                for line in lines:
                    ...
                    if line:  # если строка не пустая
                        parts = line.split(";")
                        if len(parts) == 4:  # title;author;is_taken;taken_by
                            title = parts[0]
                            author = parts[1]
                            is_taken = parts[2] == "True"
                            taken_by = parts[3] if parts[3] != "None" else None
                            
                            book = Book(title, author)
                            if is_taken and taken_by:
                                book.take_book(taken_by)
        ...
        if os.path.exists("users.txt"):
            with open("users.txt", "r", encoding="utf-8") as file:
                ...
                for line in lines:
                    ...
                    if line:
                        if ";" in line:
                            name, books_str = line.split(";", 1)
                            user = User(name)
                            if books_str:
                                taken_books = books_str.split(",")
                                for book_title in taken_books:
                                    if book_title:
    ...
    def save_data(self):
        with open("books.txt", "w", encoding="utf-8") as file:
            for book in self.__books:
                ...
                line = f"{title};{author};{is_taken};{taken_by}\n"
                file.write(line)
        ...
        with open("users.txt", "w", encoding="utf-8") as file:
            for user in self.__users:
                ...
                books_str = ",".join(taken_books)
                line = f"{name};{books_str}\n"
                file.write(line)
